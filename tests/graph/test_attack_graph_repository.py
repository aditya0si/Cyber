...
def test_login_failed_creates_attack_node(repo, env, sim_id):
    ...
    assert any(
        u == "attack_bruteforce_auth-api" and v == "auth-api" and k == "TARGETS"
        for u, v, k in kinds
    )
...
def test_get_path_returns_correct_attack_chain(repo, env, sim_id):
    ...
    # The attack chain must start at the attacker and reach the data store.
    assert paths, "expected at least one attack path from the attacker"
    assert all(p.nodes[0] == "attacker_192.168.1.100" for p in paths)
    assert any("users_db" in p.nodes for p in paths)
...
def test_response_action_block_db_updates_database_status(repo, env, sim_id):
    ...
    apply_response_actions(
        repo, sim_id, [RecommendedAction(action_id="block_database", order=1, rationale="Test")]
    )
    db_node = repo.get_node(sim_id, "users_db")
