...
from cybersim.knowledge.embeddings import TfidfEmbedder, SentenceTransformerEmbedder, _ST_AVAILABLE
...
def test_event_aggregator_produces_correct_counts(events):
    state: AnalystState = {"events": events, "org_id": "org1", "simulation_id": "demo", "window_seq": 0, "simulator_id": "web", "degraded": False}
...
def test_threat_detector_flags_brute_force_pattern(events, tools):
    state: AnalystState = {"events": events, "org_id": "org1", "simulation_id": "demo", "window_seq": 0, "simulator_id": "web", "degraded": True}
    ...
    env = build_environment_graph("web.app.sqli_login")
    proposals = analyze_window(events, org_id="org1", simulation_id="demo", window_seq=0, env=env, graph_paths=None, simulator_id="web")
    assert any(p.threat_class.value == "credential_brute_force" for p in proposals) or any(p.threat_class.value == "credential_compromise" for p in proposals)
...
def test_threat_detector_does_not_false_positive_on_normal_events(tools):
    ...
    proposals = analyze_window(normal_events, org_id="org1", simulation_id="demo", window_seq=0, env=env, graph_paths=None, simulator_id="web")
...
def test_threat_analyst_explanation_cites_real_evidence(events):
    ...
    if proposals:
        ...
        assert "events" in p.rationale.lower() or "indicator" in p.rationale.lower() or len(p.rationale) > 0
...
def test_risk_assessor_confidence_is_calculated_not_hardcoded(events):
    ...
    # Run with all events
    proposals1 = analyze_window(events, org_id="org1", simulation_id="demo", window_seq=0, env=env, graph_paths=None, simulator_id="web")
    conf1 = proposals1[0].confidence if proposals1 else 0
    
    # Run with only the first event
    proposals2 = analyze_window(events[:1], org_id="org1", simulation_id="demo", window_seq=0, env=env, graph_paths=None, simulator_id="web")
...
def test_full_pipeline_runs_with_zero_network_calls(events, repo, monkeypatch):
    ...
    outcome = runtime.ingest_window(events, org_id="org1", simulation_id="demo", window_seq=0, env=repo.graph_view("demo"))
