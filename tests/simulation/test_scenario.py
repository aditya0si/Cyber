...
def test_actor_and_target_asset_consistent_across_chain():
    ...
    scen.start(delay=0)
    events = scen.get_events()
    succ = [e for e in events if e.event_type == "LOGIN_SUCCESS"][0]
    priv = [e for e in events if e.event_type == "PRIVILEGE_ESCALATION"][0]
    db = [e for e in events if e.event_type == "DB_ACCESS"][0]
